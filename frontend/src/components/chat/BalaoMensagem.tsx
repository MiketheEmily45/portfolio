...
export default function BalaoMensagem(props: BalaoMensagemProps) {
    return props.mensagem.lado === "esquerdo" ? <BalaoEsquerdo {...props}/> : <BalaoDireito {...props}/>
}
    ...
    return (
        <div className="flex gap-4">
            ...
            <div className={`flex flex-col ${props.omitirAutor && "pl-16"}`}>
                ...
                <div className="bg-black text-white px-7 py-4 sm:w-80 rounded-r-3xl rounded-bl-3xl"><ConteudoMD markdown={props.mensagem.texto}/></div>
            </div>
        </div>
    )
}
...
function BalaoDireito(props: BalaoMensagemProps) {
    return <div className={`flex flex-col items-end ${props.omitirAutor && "pr-2"}`}>
        {!props.omitirAutor &&
            (<span className="text-xs text-zinc-600">{props.mensagem.autor}</span>
            )}
